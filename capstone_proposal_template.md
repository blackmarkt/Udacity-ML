...
A Naive Bayes Predictor model will be used to set the benchmark. A Naive predictor(6) is simply used to show what a base model without any intelligence or "naive" would look like. Since there is no clear benchmark or research paper to compare against the results will be benchmarked with random choice. The below Naive Predictor was generated in the report:

Naive Predictor: [Accuracy score: 0.5629, F-score: 0.6168]

### Evaluation Metrics

<img src="images/targets.png" width="300"/>
...
***Data Preparation***
<br>
The first step is to import all the data from their local csv files. Since assets involve time series the date range for all the data start on "12-31-17" and end "12-01-17". After all the data has been imported and collected some data exploratory analysis will be performed to detect any data that needs to be cleaned (i.e. NA's missing data). Next identify any significantly skewed data with outliers and normalize using a logarithmic transformation(8). This dataset does not conatin any categorical data so it will not be necessary to convert any non-numeric variables into categorial dummy variables through the one-hot encoding process. 

The binary categorial dependent variable will need to be generated by determining whether the current day's price was higher than the previous day's price and encoding a postiive change with a "1" and negative change with a "0". The dependent variable (output) is then separated from the independent variables (input) variables.

For both the dependent and independent data sets the training sets (X_train, y_train) will be sequentially separated from the testing sets (X_test, y_test) using a conventional 80/20 split in order to avoid look ahead bias.  

***Model Selections and Implementation***
<br> 
Luckily there are a nice selection of Supervised Learning Classifier models at our disposable and we will select at least 3 different models from the list below:
...
- Import `fbeta_score` and `accuracy_score` from [`sklearn.metrics`](http://scikit-learn.org/stable/modules/classes.html#sklearn-metrics-metrics).
 ...
 - Perform predictions on the test data `X_test` for all algorithms
...
<br>
(8)[Data Transformation](https://en.wikipedia.org/wiki/Data_transformation_(statistics)
<br>
